...
int ppc_bench() {
    ...
    if (!ppc_cpu_init()) {
        printf("cpu_init failed! Out of memory?\n");
        return 1;
    }
    ...
    for (int i = 0; i < code_size; i++) {
        ...
        if (r != PPC_MMU_OK) {
            printf("MMU error when mapping instruction address: %d\n", r);
            return 1;
        }
        ...
        *((uint32*)(gMemory+instr_physical_addr)) = instr_code_be;
        ...
        printf("Instruction %02d: 0x%08x %s\n", i, instr_code, instr_disasm);
    }
    ...
    for (int i = 0; i < test_size; i++) {
        ...
        if (r != PPC_MMU_OK) {
            printf("MMU error when mapping data address: %d\n", r);
            return 1;
        }
    ...
    for (int i = 0; i < test_samples; i++) {
        auto start_ticks   = sys_get_hiresclk_ticks();
        auto end_ticks     = sys_get_hiresclk_ticks();
        auto ticks_elapsed = end_ticks - start_ticks;
        if (ticks_elapsed < overhead) {
            overhead = ticks_elapsed;
        }
    }
    printf("Overhead Time: %lld ns\n", overhead);
    ...
    for (int i = 0; i < test_iterations; i++) {
        ...
        for (int j = 0; j < test_samples; j++) {
            ...
            auto end_ticks     = sys_get_hiresclk_ticks();
            auto ticks_elapsed = end_ticks - start_ticks;
            if (ticks_elapsed < best_sample)
                best_sample = ticks_elapsed;
        }
        uint32 iter_checksum = ppc_cpu_get_gpr(0, 3);
        if (iter_checksum != checksum) {
            printf("Checksum changed in iteration %d: 0x%08X\n", i, iter_checksum);
        }
        best_sample -= overhead;
        // Convert to ns to make comparisons with DingusPPC easier
        best_sample = best_sample * 1e9 / sys_get_hiresclk_ticks_per_second();
        printf("(%d) %lld ns, %.4lf MiB/s\n", i+1, best_sample, 1E9 * test_size / (best_sample * 1024 * 1024));
    }
